...
# Run:   docker run -d -it -v $(pwd)/fuseflow_data/results:/fuseflow-artifact/results #                              -v $(pwd)/fuseflow_data/logs:/fuseflow-artifact/logs #                              -v $(pwd)/fuseflow_data/checkpoints:/fuseflow-artifact/checkpoints #                              fuseflow-artifact bash
#
# IMPORTANT: Do NOT use --rm flag to preserve container state across crashes
...
# ============================================================================
RUN export DEBIAN_FRONTEND=noninteractive &&     apt-get update && apt-get install -y     build-essential     ninja-build     clang     lld     python3     python3-dev     python3-pip     python3-venv     graphviz     git     curl     wget     unzip     pkg-config     libssl-dev     && rm -rf /var/lib/apt/lists/*

RUN wget https://github.com/Kitware/CMake/releases/download/v3.28.3/cmake-3.28.3-linux-x86_64.sh -O /tmp/cmake-install.sh &&     chmod +x /tmp/cmake-install.sh &&     /tmp/cmake-install.sh --skip-license --prefix=/usr/local &&     rm /tmp/cmake-install.sh
...
# ============================================================================
RUN wget https://github.com/protocolbuffers/protobuf/releases/download/v24.0/protoc-24.0-linux-x86_64.zip     && unzip protoc-24.0-linux-x86_64.zip -d /usr/local     && rm protoc-24.0-linux-x86_64.zip
...
RUN chmod +x scripts/patch_pytorch_libs.sh &&     bash scripts/patch_pytorch_libs.sh
...
# ============================================================================
RUN cd comal &&     maturin develop --release

# ============================================================================
# Stage 11: Build Tortilla visualizer
# ============================================================================
RUN cd tortilla-visualizer &&     make

# ============================================================================
# Stage 12: Create directories for persistent data
# ============================================================================
RUN mkdir -p /fuseflow-artifact/results &&     mkdir -p /fuseflow-artifact/logs &&     mkdir -p /fuseflow-artifact/checkpoints &&     chmod -R 755 /fuseflow-artifact

VOLUME ["/fuseflow-artifact/results", "/fuseflow-artifact/logs", "/fuseflow-artifact/checkpoints"]
